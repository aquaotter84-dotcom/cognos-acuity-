...
| **27** | **BUILT.** The destination-grant KEY PATH: the goal form carries an optional `{ effect: "webhook.post", destinations: [...] }` grant into scope; the designer create accepts `grant_destinations`; `validateDestinationGrant` refuses malformed grants with named sentences at the only moment they can still be fixed (scope is immutable), and a grant lands only with `webhook.post` surviving the clamp; `test/phase27.mjs` (6). No new laws. | **Rung 4** now earnable from the operator's hand: `corpus_aimed` can be met from the UI, and the aimed corpus can fill |
| **28** | **BUILT.** The earned-corpus bypass as a DELEGATED operator switch: `autonomy_settings.bypass_earning` (migration `0017`), `COGNOS_AUTONOMY_BYPASS_EARNING_UI_CONTROL` delegation, `COGNOS_AUTONOMY_BYPASS_EARNING` pin (legacy `_BYPASS_EVIDENCE` alias kept), refusal sentences, write-through cache and an `autonomy.bypass_earning` audit row — the off-ramp that shipped env-only is now a toggle on the Autonomy page. What it waives is exactly the shadow corpus: the rung flag, the one approved destination, autonomy-on, quiet hours and the per-effect Governor all still bind, and T5 still releases only by a per-effect human approval. Also fixes a latent cache drop: an enable/disable flip no longer forgets the mode, auto-authorize or the bypass. `test/phase28.mjs` (7). No new laws. | The one gate that could only be opened by a restart is now opened from the page; the corpus becomes a default rather than a wall |
| **29** | **BUILT.** The RUNG switches as DELEGATED operator switches: five nullable columns on `autonomy_settings` (`rung_residents`, `rung_search`, `rung_external_writes`, `rung_irreversible`, `rung_inbound`; migration `0018`), one delegation (`COGNOS_AUTONOMY_RUNGS_UI_CONTROL`), the five existing variables kept as tri-state PINS that outrank the page in both directions, per-rung refusal sentences, write-through cache and an `autonomy.rung` audit row. `config.rung` is a RESOLVED value now instead of a raw environment read, so `tierAllowed` and the Governor's rung gate ask the same question they always did and get the page's answer. One column per rung so two flips cannot lose one another's write. A rung is one of two gates: the shadow corpus and T5's per-effect approval are still not writable from any request, so opening a rung never releases anything by itself. `test/phase29.mjs` (12). No new laws. | Rung 4, 5 and 6 reachable without a restart; the environment keeps a veto, never the only hand on the switch |
