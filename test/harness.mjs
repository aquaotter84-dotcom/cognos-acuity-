...
export async function bootHarness(env = {}) {
  ...
  async function chat(userMessage, { conversationId = null, style = "balanced", webSearch = false, attachments = [], agentMode = "off", researchRunId = null, projectId = null, goalId = null } = {}) {
    const t0 = Date.now();
    const res = await fetch(`${base}/api/chat`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ userMessage, conversationId, style, webSearch, attachments, agentMode, researchRunId, projectId, goalId })
    });
    if (!res.ok || !res.body) {
